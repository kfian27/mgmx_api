...
exports.queryPosisiHutang = async (companyid, tanggal) => { 
    ...
    if (companyid == fun.companyWI) {
        ...
        SELECT IdMSup 
            ...
            SELECT IdMSup 
                , 3.1 as JenisTrans 
                , IdTBeliAset as IdTrans 
                , BuktiTBeliAset as BuktiTrans 
                , concat(Date(TglTBeliAset), ' ', Time(TglUpdate)) as TglTrans 
                , JmlBayarKredit AS JmlHut 
                , concat('Pembelian Aset ', BuktiTBeliAset) as Keterangan
                , IdMCabang
            FROM MGAPTBeliAset 
            WHERE Hapus = 0 
            AND Void = 0 
...
exports.queryKartuHutang = async (companyid, start, end) => { 
    ...
    if (companyid == fun.companyWI) {
        ...
        FROM (
        SELECT IdMSup, 0 As Urut, 0 as JenisTrans, 0 as IdTrans, '-' As BuktiTrans, cast('${start} 00:00:00' as DateTime) As TglTrans, 0 As JmlHut, sum(JmlHut) As Saldo, 'Saldo Sebelumnya' As Keterangan FROM (
        ...
        as LKartuHut WHERE TglTrans < '${start} 00:00:00' GROUP BY IdMSup
        ) TableSaldoAwal
        GROUP BY IdMSup
        UNION ALL
