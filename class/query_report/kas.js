exports.queryPosisiKas = async (date, idcompany) => {
    var sql = "";
    if(idcompany == 5){
...
exports.queryKartuKas = async (start, end, mkas) => {
    let qkas = "";
    if (mkas != "") {
...
FROM (
SELECT IF(Sum(JmlKas) >= 0, 1, 2) AS IsDebetKredit, IdMCabang,  IdMKas, 0 As Urut, 0 as JenisTrans, 0 as JenisRugiLaba, 0 as IdTrans, '-' As BuktiTrans, cast('2024-03-01 00:00:00' as Date) As TglTrans, 0 As JmlKas, sum(JmlKas) As Saldo, concat('Saldo Awal per ',cast('2024-03-01 00:00:00' as Date)) As Keterangan, '-' as noref 
 FROM (
 SELECT IdMCabang, IdMKas,  0 As JmlKas FROM MGKBMKas
 UNION ALL
 SELECT IdMCabang, IdMKas, JmlKas FROM MGKBLKartuKas where CAST(TglTrans as DATE) < CAST('2024-03-01 00:00:00' AS DATE)
) TableSaldoAwal
GROUP BY IdMCabang, IdMKas
UNION ALL
SELECT IF(JmlKas >= 0, 1, 2) AS IsDebetKredit, IdMCabang, IdMKas, 1 as Urut, JenisTrans, JenisRugiLaba, IdTrans, BuktiTrans, TglTrans, JmlKas, 0, Keterangan, noref
FROM MGKBLKartuKas where CAST(TglTrans as DATE) >= CAST('2024-03-01 00:00:00' AS DATE) and CAST(TglTrans as DATE) < CAST('2024-04-01 00:00:00' AS DATE)
) TableKartuKas LEFT OUTER JOIN MGSYMCabang MCabang ON (TableKartuKas.IdMCabang = MCabang.IdMCabang)
               LEFT OUTER JOIN MGKBMKas MKas ON (TableKartuKas.IdMCabang = MKas.IdMCabang AND TableKartuKas.IdMKas = MKas.IdMKas)
WHERE MCabang.Hapus = 0
 AND MCabang.Aktif = 1
